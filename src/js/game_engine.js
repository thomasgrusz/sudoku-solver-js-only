...
function solveSudoku() {
  ...
  if (
    !Object.values(sudokuGrid).every(
      (char) => [...validCharacters].includes(char) || char === validCharacters
    )
  ) {
    injectAlert("Only numbers from 1-9!");
    return;
  } else if (
    Object.values(sudokuGrid).every((char) => char === validCharacters)
  ) {
    return;
  }
  // Send string to search() function to solve the puzzle (transform to grid values beforehand)
  const sudokuSolved = search(sudokuGrid);
  ...
  if (document.getElementById("error").firstChild) {
    // Remove alert, if existing
    document
      .getElementById("error")
      .removeChild(document.getElementById("error").firstChild);
  }
  const originalFields = [...tdElements]
    .filter((td) => td.value.length === 1)
    .map((td) => td.id); // Get original user input fields
  originalFields.forEach((field) =>
    document.getElementById(field).classList.add("fw-bold")
  ); // Make original user input bold
  ...
  document
    ...
    .firstChild.addEventListener("click", startSudokuSolver);
}

// Function to start the Sudoku Solver
function startSudokuSolver() {
  createSudokuGrid();
  createButton("SOLVE", "btn btn-success my-4");
  // Add Eventlistener to solve-button
  document
    .getElementById("solve-button")
    .firstChild.addEventListener("click", solveSudoku);
